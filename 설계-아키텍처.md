...
### 2.1 레이어 구조 (헤드리스 아키텍처)

```
┌─────────────────────────────────────────┐
│           Framework Adapters            │  ← React/Vue/Svelte 어댑터 (UI 렌더링 담당)
├─────────────────────────────────────────┤
│            Command System               │  ← 사용자 액션을 Transaction으로 변환
├─────────────────────────────────────────┤
│           Decoration System             │  ← 데이터 기반 UI 힌트 제공 (DOM 조작 없음)
├─────────────────────────────────────────┤
...
**핵심 변경사항:**
- **View Layer 제거**: DOM 조작을 코어에서 제거하여 완전한 헤드리스 구현
- **Framework Adapters 강화**: 각 프레임워크가 데이터를 받아 직접 UI 렌더링
- **Decoration System 개선**: DOM 조작 대신 데이터만 제공

### 2.2 데이터 플로우 (헤드리스 패턴)

```
User Interaction → Framework Adapter → Command → Transaction → State Update → Decoration Data → Framework Re-render
```

**헤드리스 패턴의 특징:**
- 코어는 DOM을 직접 조작하지 않음
- 모든 UI 렌더링은 프레임워크 어댑터가 담당
- 데코레이션은 렌더링 힌트 데이터만 제공
...
### 3.5 헤드리스 View System

#### 3.5.1 CalendarView 클래스 (헤드리스 버전)
...
class CalendarView {
  ...
  dispatch(transaction: Transaction): void {
    ...
    // 4. 상태 적용 및 데코레이션 업데이트
    this.state = newState;
    this.updateDecorations();

    // 5. 상태 변경 알림 (프레임워크 어댑터에서 리렌더링)
  ...
  private updateDecorations(): void {
    const decorations = this.plugins
      .map(plugin => plugin.spec.decorations?.(this.state))
      .filter(Boolean) as DecorationSet[];
    
    const allDecorations = decorations.reduce(
      (acc, set) => acc.addAll(set.decorations),
      new DecorationSet()
    );
    
    this.decorationManager.updateDecorations(allDecorations);
  }

  // 데코레이션 데이터 제공
  getDecorations(): DecorationSet {
    return this.decorationManager.getCurrentDecorations();
  }
...
private updateDecorations(): void {
    ...
    // 데코레이션 매니저에 데이터 저장 (DOM 조작 없음)
    this.decorationManager.updateDecorations(allDecorations);
}

// 프레임워크 어댑터에서 사용할 데이터 제공
getDecorationMap(): Map<string, Decoration[]> {
    return this.decorationManager.getDecorationMap();
}

getDecorationsForDate(date: Date): Decoration[] {
    return this.decorationManager.getDecorationsForDate(date);
}

// 이벤트 처리도 헤드리스 방식으로 변경
handleDateClick(date: Date, nativeEvent?: MouseEvent): boolean {
    return this.plugins.some(plugin => 
        plugin.spec.props?.handleDateClick?.(date, nativeEvent, this.state)
    );
}
```
