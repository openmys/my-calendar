...
import { CalendarState, CalendarDay, PluginState, ViewType, TimeRange } from '@/types';
...
export class CalendarStateFactory {
   ...
   * 기본 CalendarState 생성
   */
  static createInitialState(options: {
    currentDate?: Date;
    viewType?: ViewType;
    timeRange?: TimeRange;
    timezone?: string;
   ...
   */
  static create(plugins: any[] = [], options?: {
    currentDate?: Date;
    viewType?: ViewType;
    timeRange?: TimeRange;
    timezone?: string;
...
export class StateUpdater {
  ...
  static updateCurrentDate(state: CalendarState, newDate: Date): CalendarState {
    const newTimeRange = CalendarStateFactory.calculateTimeRangeForView(newDate, state.viewType);
   ...
   * 뷰 타입 변경
   */
  static updateViewType(state: CalendarState, viewType: ViewType): CalendarState {
    const newTimeRange = CalendarStateFactory.calculateTimeRangeForView(state.currentDate, viewType);
   ...
   * 월 이동 (다음/이전)
   */
  static navigateMonth(state: CalendarState, direction: 'next' | 'previous'): CalendarState {
   ...
   * 주 이동
   */
  static navigateWeek(state: CalendarState, direction: 'next' | 'previous'): CalendarState {
    const currentDate = new Date(state.currentDate);
   ...
   * 일 이동
   */
  static navigateDay(state: CalendarState, direction: 'next' | 'previous'): CalendarState {
    const currentDate = new Date(state.currentDate);
...
export class StateQuery {
   ...
   * 플러그인 상태 가져오기
   */
  static getPluginState<T = any>(state: CalendarState, pluginId: string): PluginState<T> | undefined {
