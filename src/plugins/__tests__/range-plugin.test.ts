...
describe('Range Plugin', () => {
  ...
  describe('Range Selection', () => {
    it('should start range selection', () => {
      ...
      const transaction = transactions.custom('RANGE_START_SELECTION', { date: startDate });
    ...
    it('should end range selection', () => {
      ...
      // 선택 시작
      state = rangePlugin.spec.state!.apply(
        transactions.custom('RANGE_START_SELECTION', { date: new Date('2023-06-10') }),
        state
      );
    ...
    it('should select range directly', () => {
      ...
      const newState = rangePlugin.spec.state!.apply(
        transactions.custom('RANGE_SELECT_RANGE', { start: startDate, end: endDate }),
        state
      );
  ...
  describe('Hover Functionality', () => {
    ...
    it('should clear hovered date', () => {
      ...
      // 호버 설정
      state = rangePlugin.spec.state!.apply(
        transactions.custom('RANGE_HOVER_DATE', { date: new Date('2023-06-15') }),
        state
      );
  ...
  describe('Clear Selection', () => {
    it('should clear all selection state', () => {
      ...
      state = rangePlugin.spec.state!.apply(
        transactions.custom('RANGE_HOVER_DATE', { date: new Date('2023-06-20') }),
        state
      );
  ...
  describe('Queries', () => {
    ...
    it('should get selected range', () => {
      ...
      const selectedRange = rangePlugin.spec.queries!.getSelectedRange(mockState, rangePlugin);
    ...
    it('should get selected dates from range', () => {
      ...
      const selectedDates = rangePlugin.spec.queries!.getSelectedDates(mockState, rangePlugin);
    ...
    it('should check if date is selected', () => {
      ...
      expect(rangePlugin.spec.queries!.isDateSelected(mockState, rangePlugin, middleDate)).toBe(true);
      expect(rangePlugin.spec.queries!.isDateSelected(mockState, rangePlugin, outsideDate)).toBe(false);
    });
    
    it('should get selection mode', () => {
      const mode = rangePlugin.spec.queries!.getSelectionMode(mockState, rangePlugin);
    ...
    it('should check if currently selecting', () => {
      ...
      expect(rangePlugin.spec.queries!.isSelecting(mockState, rangePlugin)).toBe(false);
      ...
      expect(rangePlugin.spec.queries!.isSelecting(mockState, rangePlugin)).toBe(true);
    });
  });
  ...
  describe('Serialization', () => {
    it('should serialize and deserialize state correctly', () => {
      ...
      expect(deserialized.value.selectedRange).toEqual(state.value.selectedRange);
      expect(deserialized.value.selectedDates).toEqual(state.value.selectedDates);
