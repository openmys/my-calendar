...
export interface PluginFactoryOptions<T = BaseCustomPluginState> {
  ...
  /** 이벤트 핸들러들 */
  eventHandlers?: {
    onDateClick?: (date: Date, event: MouseEvent, state: CalendarState, pluginState: T) => boolean;
    onTimeClick?: (datetime: Date, event: MouseEvent, state: CalendarState, pluginState: T) => boolean;
    onKeyDown?: (event: KeyboardEvent, state: CalendarState, pluginState: T) => boolean;
    onKeyUp?: (event: KeyboardEvent, state: CalendarState, pluginState: T) => boolean;
  };
  
  /** 트랜잭션 필터 */
  transactionFilter?: (transaction: Transaction, state: CalendarState) => boolean;
  
  /** 트랜잭션 후처리 */
  transactionPostProcessor?: (transactions: Transaction[], oldState: CalendarState, newState: CalendarState) => Transaction | null;
}
...
class CustomPluginState<T = BaseCustomPluginState> extends BasePluginState<T> {
  private stateHandlers: { [key: string]: (state: T, payload: unknown) => Partial<T> };
  ...
  static fromJSON<T>(value: T, handlers: Record<string, any> = {}): CustomPluginState<T> {
...
export function createCustomPlugin<T = BaseCustomPluginState>(
  ...
  const spec: PluginSpec<T> = {
    ...
    // 데코레이션
    decorations: decorationFactory ? (state: CalendarState, plugin: Plugin<T>) => {
      const pluginState = plugin.getState(state);
      return pluginState ? decorationFactory(state, pluginState.value) : new DecorationSet();
    } : undefined,
    ...
    props: {
      handleDateClick: eventHandlers.onDateClick ? (
        ...
        const pluginState = plugin.getState(state);
        return pluginState ? eventHandlers.onDateClick!(date, event, state, pluginState.value) : false;
      } : undefined,
      ...
      handleTimeClick: eventHandlers.onTimeClick ? (
        ...
        const pluginState = plugin.getState(state);
        return pluginState ? eventHandlers.onTimeClick!(datetime, event, state, pluginState.value) : false;
      } : undefined,
      ...
      handleKeyDown: eventHandlers.onKeyDown ? (
        ...
        const pluginState = plugin.getState(state);
        return pluginState ? eventHandlers.onKeyDown!(event, state, pluginState.value) : false;
      } : undefined,
    ...
    return createCustomPlugin({
      ...
      stateHandlers: {
        ...
        'REMOVE_HIGHLIGHT_DATE': (state, payload) => ({
          highlightDates: (state as any).highlightDates.filter((date: Date) => 
            date.getTime() !== payload.date.getTime()
          )
        }),
      ...
      decorationFactory: (_state, pluginState) => {
        const decorations = (pluginState as any).highlightDates.map((date: Date) => ({
    ...
    return createCustomPlugin({
      ...
      stateHandlers: {
        ...
        'REMOVE_DISABLED_DATE': (state, payload) => ({
          disabledDates: (state as any).disabledDates.filter((date: Date) => 
            date.getTime() !== payload.date.getTime()
          )
        }),
      ...
      decorationFactory: (_state, pluginState) => {
        const decorations: Array<{ type: DecorationType; from: Date; spec: Record<string, unknown> }> = [];
      ...
      eventHandlers: {
        onDateClick: (date, event, _state, pluginState) => {
          // 비활성화된 날짜 클릭 차단
          const dateKey = date.toISOString().split('T')[0];
          const isDisabled = (pluginState as any).disabledDates.some((d: Date) => 
          ...
          // 주말 체크
          if ((pluginState as any).disableWeekends && (date.getDay() === 0 || date.getDay() === 6)) {
            event?.preventDefault();
            return true;
          }
...
export namespace PluginHelpers {
  ...
  export const StateHelpers = {
    ...
    addToArray: (key: string) => (state: Record<string, unknown>, item: unknown) => ({
